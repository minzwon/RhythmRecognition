...
class Eval:
    ...
    def get_path(self, path_type):
        if path_type == 0:
            self.path = '/data1/ExBallroom/spec_trp/'
        else:
            self.path = '/data1/ExBallroom/'+str(path_type)+'p_spec_trp/'
    ...
    def run(self, gpu_id=0, path_type=0):
        ...
        #n = Network()
        #self.model = n.model
        #self.model.load_weights('checkpoints/spec_notempo.15.hdf5')
        self.model = load_model('spec_minz.keras')
        self.corr = 0
